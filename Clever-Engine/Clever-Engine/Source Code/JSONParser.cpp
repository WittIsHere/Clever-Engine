...
{
	if (json_object_set_number(rootNode, name, number) != JSONFailure)
		JSONSuccess;
	else
		LOG("[ERROR] JSON Parser: Could not set { %s } with the given Integer!", name);
}
...
{
	if (json_array_append_number(jsonArray, number) != JSONFailure)
		++size;
	else
...
{
	if (json_array_append_string(jsonArray, string) != JSONFailure)
		++size;
	else
...
{
	if (json_array_append_boolean(jsonArray, value) != JSONFailure)
		++size;
	else
...
{
	if (json_array_append_number(jsonArray, (double)color.r) != JSONFailure)
		++size;
	else 
		LOG("[ERROR] JSON Parser: Could not append R to { %s } Array!", name);
	if (json_array_append_number(jsonArray, (double)color.g) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append G to { %s } Array!", name);
	if (json_array_append_number(jsonArray, (double)color.b) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append B to { %s } Array!", name);
	if (json_array_append_number(jsonArray, (double)color.a) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append A to { %s } Array!", name);
}

void ParsonArray::SetFloat3(const math::float3& vec3)
{
	if (json_array_append_number(jsonArray, (double)vec3.x) != JSONFailure)
		++size;
	else 
		LOG("[ERROR] JSON Parser: Could not append X to { %s } Array!", name);
	if (json_array_append_number(jsonArray, (double)vec3.y) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append Y to { %s } Array!", name);
	if (json_array_append_number(jsonArray, (double)vec3.z) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append Z to { %s } Array!", name);
}

void ParsonArray::SetFloat4(const math::float4& vec4)
{
	if (json_array_append_number(jsonArray, (double)vec4.x) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append X to { %s } Array!", name);
	if (json_array_append_number(jsonArray, (double)vec4.y) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append Y to { %s } Array!", name);
	if (json_array_append_number(jsonArray, (double)vec4.z) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append Z to { %s } Array!", name);
	if (json_array_append_number(jsonArray, (double)vec4.w) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append W to { %s } Array!", name);
}

ParsonNode ParsonArray::SetNode(const char* name)
{	
	if (json_array_append_value(jsonArray, json_value_init_object()) != JSONFailure)
		++size;
	else
		LOG("[ERROR] JSON Parser: Could not append Node to { %s } Array!", this->name);
